...
The binding constraint is coverage economics, not correctness - but German compound-head matching makes that constraint softer than the raw numbers suggest. The 35% dictionary recall penalizes per-entry against a dictionary that enumerates compound spellings, while our unit of coverage is the head: one imported head rule covers its open compound family with inflection handled through the declension tables (verified live: Berufskraftfahrer via Fahrer, Chefärzte via Arzt). The 324-term gap list reduces to 260 editorial units on that basis, and German coverage grows per-family rather than per-word through the rule editor's evaluation/import loop, which now has license-clean input (the CC0 subsets). The multiplier is conditional - twelve covered heads whose compounds still missed show it depends on rule flags and noun-table wiring - and French has no equivalent (compounds there are multiword). The measured boundary of the algorithm is context-dependent rewriting - INCLURE's pronoun/participle/agreement class (86% of their gold) and pair formulas - which rule-per-lemma cannot reach; if that class becomes commercially relevant, the paths are seq2seq fine-tuning on INCLURE's MIT/CC0 pairs or the product's existing LLM-alternatives channel, as an additive layer whose output the deterministic core can check before display.
...
| maintenance unit | a rule: data edit + versioned evaluation + DB sync, minutes, non-engineers; a German head rule covers its whole compound family | per surface string - compounds enumerated one by one | retraining cycle: data, GPU, eval infra, regression risk | prompt tuning + vendor drift |
