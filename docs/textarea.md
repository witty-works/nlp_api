...
- **Length.** Witty checks up to `TEXT_MAX_LENGTH` characters of a text at once (1000 by default), for the underlines as for a prompt's draft. The model is asked to keep a draft within that, and the page says so when a draft is longer and was only checked at its start. A prompt can rewrite a text of up to 4000 characters.
